...
const Footer = ({ app }) => {
  return (
    <footer className="bg-base-200 border-t-2 bg-opacity-30">
      ...
      <div className="flex flex-row flex-wrap justify-center gap-12 pb-6 pt-4">
        {order.map(partner => {
      ...
      <div className="text-center pt-12 pb-8 flex flex-row gap-6 items-center justify-center text-lg">
        <Link href="/about"><a>{app.navigation.about.__title}</a></Link>
        <span>|</span>
        <Link href="/info"><a>{app.navigation.info.__title}</a></Link>
        <span>|</span>
        <Link href="/contact"><a>{app.navigation.contact.__title}</a></Link>
      </div>
